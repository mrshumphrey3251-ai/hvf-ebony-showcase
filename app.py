...
A: Starts at $1.99/acre/month (Basic). Pro adds multispectral for $2.99/acre/month.''')
    ...
    if current_role in ["CEO", "SUPER_ADMIN"]:
        ...
        # ADDITIVE MFA LOCK INJECTION
        if "mfa_unlocked_engine" not in st.session_state:
            st.session_state.mfa_unlocked_engine = False
            
        if not st.session_state.mfa_unlocked_engine:
            st.warning("🔒 KINETIC LOCK: Tier-1 CEO Token Required to Arm Engine.")
            mfa_attempt = st.text_input("Enter 6-Digit CEO Token:", type="password", key="mfa_engine_input")
            if st.button("Authenticate to Arm Engine"):
                from core.mfa_matrix import verify_mfa_token
                if verify_mfa_token(mfa_attempt, seed="EBONY_TIER_1_CEO"):
                    st.session_state.mfa_unlocked_engine = True
                    st.rerun()
                else:
