...
if 'user' not in st.session_state: st.session_state.user = 'JEFFERY HUMPHREY'
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**NODE STATUS:** ACTIVE (TPM 2.0 VERIFIED)")
    st.markdown("**UPLINK:** SECURE (TLS-PSK)")

st.markdown("## 🔴 EBONY: BARE-METAL COMMAND NODE")
st.markdown("---")

if st.session_state.role in ['CEO', 'COMMANDER']:
    st.success("STATUS: TPM-ROOTED UPLINK SECURED. DUAL-GPU MATRIX ONLINE.")
    
    st.markdown("### 🌾 AGRICULTURE: SECTOR ALPHA")
    a1, a2, a3, a4 = st.columns(4)
    with a1: st.metric("Multi-Index Fusion Vigor", "0.92", "+0.01", delta_color="normal")
    with a2: st.metric("Soil Matric Potential", "Optimal", "0 L/min flow")
    with a3: st.metric("Dual-GPU Load (Active)", "16%", "Standby: 0%")
    ...
    st.warning("WARNING: Activating these switches will bypass autonomous loops and execute immediate physical reactions. All overrides are logged to the immutable Hyperledger.")
    ...
    st.error("🔒 ACCESS DENIED. Hardware-Rooted Executive clearance required to view live telemetry and operate kinetic execution switches.")
